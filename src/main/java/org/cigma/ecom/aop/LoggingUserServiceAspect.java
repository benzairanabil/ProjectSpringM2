...
public class LoggingUserServiceAspect {
    ...
    @Before("execution(* org.cigma.ecom.service.UserServiceImp.*(..))")
    public void logBeforeMethodExecution(JoinPoint joinPoint) {
        logger.info("Preparing to execute method {} with arguments {}", joinPoint.getSignature().getName(), joinPoint.getArgs());
    }

    @After("execution(* org.cigma.ecom.service.UserServiceImp.*(..))")
    public void logAfterMethodExecution(JoinPoint joinPoint) {
        logger.info("Successfully executed method {}", joinPoint.getSignature().getName());
    }
    
    @AfterReturning(pointcut = "execution(* org.cigma.ecom.service.UserServiceImp.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        logger.info("Method {} returned {}", joinPoint.getSignature().getName(), result);
    }

    @AfterThrowing(pointcut = "execution(* org.cigma.ecom.service.UserServiceImp.*(..))", throwing = "exception")
